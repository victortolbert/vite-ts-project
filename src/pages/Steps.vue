    ...
    <div>
      <div v-for="(segment, i) in paths.segments" :key="i">
        <span v-if="!segment.state.matches(paths.segments[i - 1]?.state.value)">
          On the
          <strong>{{ getDescription(segment) }}</strong>,
        </span>
        <span>
          <span v-if="segment.state.matches(paths.segments[i - 1]?.state.value)">... and then</span>
          <strong
            v-if="getTranstions(segment)[0].meta?.description"
          >{{ getTranstions(segment)[0].meta?.description }}</strong>
          <span v-else>
            Execute event
            <strong>{{ transition.eventType }}</strong>
          </span>
          <span
            v-if="getTranstions(segment)[0].cond?.meta?.description"
          >({{ getTranstions(segment)[0].cond?.meta?.description }})</span>
        </span>
      </div>
      <div>
        Then you will reach
