  ...
  <section>
    <ul>
      <li :key="address.id" v-for="address in addresses">
        <label class="field -radio" :data-checked="isSelectedShippingAddress(address.id)">
          <div>
    ...
    <div v-show="state.context.shipping.id">
      ...
      <ul>
        <li :key="method.value" v-for="method in shippingMethods">
          <label class="field -radio" :data-checked="isSelectedShippingMethod(method.value)">
