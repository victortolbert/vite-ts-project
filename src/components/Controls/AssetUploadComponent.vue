...
import { Asset, AssetType, AssetUpload, AssetUploadModel, ExemplarMessage, EventBus, PageBase, PifEvents } from '@/types'
...
export default class AssetUploadComponent extends PageBase<Asset> {
  ...
  onUpload(e: any) {
    ...
    e.data = {
      assetFullPath: `${this.assetVaultPath}/${this.uploadModel.CompanyId}/${this.uploadModel.ProjectId}/`,
      fileName: fileName,
      companyId: this.uploadModel.CompanyId,
      projectId: this.uploadModel.ProjectId,
      assetTypeId: this.uploadModel.AssetTypeId,
    }
  }
  ...
  async onSuccess(e: any) {
    if (<boolean>e.response.IsSuccess) {
      ...
      if (this.uploadModel.AssetTypeId == AssetType.FieldAsset) {
...
<template>
  <div v-if="uploadModal.open">
    <b-modal
      id="uploadModal"
      v-model="uploadModal.open"
      header-class="card-header"
      :no-close-on-backdrop='uploadModal.noCloseOnBackdrop'
      :no-close-on-esc='uploadModal.noCloseOnEsc'
      v-on:hide="onCloseModal"
      size="md"
    >

      <div slot="modal-title">
        <span class="project">Upload {{ assetType }}</span>
      </div>

      <div v-if="showDescription" style="color: black; padding: 10px; margin:10px">
        Description: <input v-model="description" class="form-control formValue tb">
      </div>
      ...
      <kendo-upload
        ...
        :validation-allowed-extensions='["jpg","jpeg","png","pdf","eml","msg","xls","xml","xlsx","dox","docx","ppt","pptx","zip","esx"]'
      />
      ...
      <div slot="modal-footer">
        <b-button variant="primary" @click.prevent="onCloseModal">
          Close
        </b-button>
      </div>
    </b-modal>
