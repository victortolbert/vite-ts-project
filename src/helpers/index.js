...
function createPagingRange(nrOfPages, currentPage) {
  ...
  for (let i = 0; i < range.length; i++) {
    if (length) {
      if (range[i] - length === 2) rangeWithDots.push(length + 1)
      else if (range[i] - length !== 1) rangeWithDots.push(MORE_PAGES)
    }
    ...
    items = items.filter(item => {
      ...
      for (const itemKey in itemValue) {
        if (itemKey === filterKey) {
          ...
          if (filterFields[filterKey] === '') return true
          if (itemValue[itemKey] === filterFields[filterKey]) return true
        }
      }
      return false
    })
...
function findAny(dsSearchIn, dsSearchAs, obj, str) {
  ...
  for (const key in obj) {
    if (dsSearchIn.length === 0 || dsSearchIn.includes(key)) {
      ...
      if (value.includes(str)) return true
    }
  }
  return false
}
